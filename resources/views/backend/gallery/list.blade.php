                            ...
                            <tbody>
                                @foreach ($datas as $key => $data)
                                    <tr>
                                        ...
                                        <td>
                                            @if (empty($data->type))
                                                <img width="50" class="mr-3"
                                                    src="{{ asset('normal_images/' . $data->image) }}">
                                            @else
                                                <a target="_blank"
                                                    href="{{ asset('normal_images/' . $data->image) }}">Download</a>
                                            @endif
                                        </td>

                                        <td>{{ ucfirst($data->type) }}</td>
                                        <td>{{ $data->data_filter }}</td>
                                        ...
                                        <td class="table-action">
                                            <a href="{{ route('gallery.edit', $data->id) }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    ...
                                                    title="Edit" class="feather feather-edit-2 align-middle">
                                        ...
                                        <td class="table-action">
                                            {{-- <a href="#">
                                        <svg  class="badge bg-danger" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash align-middle">
                                            <polyline points="3 6 5 6 21 6">
                                                </polyline>
                                                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                                            </svg>
                                        </a> --}}
                                            <form action="{{ url('admin/gallery' , $data->id ) }}" method="POST">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                <button type="submit" onclick="return confirm('Are you sure, want to delete this Image?')" width="24" height="24" data-toggle="tooltip" title="Trash" class="pull-right btn-sm btn btn-danger"><i class="fa fa-trash"aria-hidden="true"></i></button>
                                {{-- <button type="submit" onclick="return confirm('Are you sure, want to delete this Lead?')" class="btn btn-danger">Delete</button> --}}
                                            </form>
                                        </td>
                                        {{--  --}}
                                    </tr>
